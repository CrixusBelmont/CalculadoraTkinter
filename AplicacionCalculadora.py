...
class Calculadora(tk.Tk):
    ...
    def _creacion_componentes(self):
        ...
        # Primer renglón
        # Botón limpiar
        boton_limpiar = tk.Button(botones_frame, text='AC', width=10, height=3,
                                  bd=0, bg='#eee', cursor='hand2',
                                  command=self._evento_limpiar)
        boton_limpiar.grid(row=0, column=0, padx=1, pady=1)
        # Botón dividir
        ...
        #boton (
        boton_parentiz = tk.Button(botones_frame, text='(', width=10, height=3, bd=0, bg='#fff',
                                   cursor='hand2', command=lambda: self._evento_click("("))
        boton_parentiz.grid(row=0, column=1, padx=1, pady=1)
        #boton )
        boton_parentde = tk.Button(botones_frame, text=')', width=10, height=3, bd=0, bg='#fff',
                                   cursor='hand2', command=lambda: self._evento_click(")"))
        boton_parentde.grid(row=0, column=2, padx=1, pady=1)
...
#El lambda en este caso significa que estas definiendo una funcion anonima
#Es decir que la funcion no se mandará a llamar automaticamente o de inmediato
#Sino que dicha funcion con el lambda solo se mandará a llamar cuando presionemos el boton
